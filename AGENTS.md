...
`hermes kanban --board bantubayar list`
<!-- roadmap-pointer:end -->
...
## Bantubayar (project-specific)

Laravel 12 + Blade (server-rendered, no SPA) app for handling unpaid student tuition ("penanganan pembayaran") at Pesantren Daruttaqwa. PHP 8.4, Node 22, Vite + Tailwind v4, Pest for tests, spatie/laravel-permission for authz.

### Data model — the most important thing to understand

- **Student/payment data does NOT live in the `bantubayar` database.** It lives in external MySQL schemas on the same `lerd-mysql` server, queried via cross-database SQL:
  - `daruttaqwa_trans` (tuition journal: `ips_siswa`, `tbl_ips_unit`, `tbl_ips_main`)
  - `daruttaqwa_referensi` (`tbl_departemen`, `tbl_periode`)
  - `daruttaqwa_sisda` (`tbl_siswa`, `tbl_kelas`)
...
### Home visit flow (pengajuan → approval → laporan)

- A home visit is now a **request/approval workflow**, not an admin-only form:
  1. **Pengajuan** (bendahara/petugas) from the student's penanganan page (`penanganan/show`). The "Home Visit" button opens a modal that calls `GET penanganan/home-visit/status/{id_siswa}` to fetch eligibility, then `POST penanganan/home-visit/ajukan`. The new row starts at status `pending`.
  2. **Approval** (admin) via `POST admin/home-visit/{id}/approve` or `/reject`; admin can also `/batal`.
  3. **Laporan** by the home-visit officer **without login** through the single combined link `/visit/{token}` — the page shows the task detail *and* the report form. The link only works once status is `disetujui`/`dilaksanakan`; opening it flips `disetujui → dilaksanakan`, and submitting sets `selesai` and writes a `PenangananHistory` row with `jenis_penanganan = 'home_visit'`.
- Status enum on `home_visits`: `pending`, `disetujui`, `ditolak`, `dijadwalkan`, `dilaksanakan`, `selesai`, `batal`. `petugas_nama` / `petugas_hp` are **nullable** — filling them is optional; otherwise just share the link.
- `App\Services\HomeVisitEligibilityService::evaluate($siswa)` is the single source of truth for the "sulit"/eligible gate. It inspects the active penanganan's histories (chat/phone counts), `hasil` (`tidak_ada_respon`, `hp_tidak_aktif`), age since `updated_at` (mandeg ≥ 7 days), missing wali phone, and overdue `kesanggupanTerakhir`. It returns `{eligible, syarat_terpenuhi[], syarat_belum[], boleh_override, penanganan_id}`. A manual override is allowed when an `alasan_override` string is supplied.
- New columns on `home_visits`: `penanganan_id`, `diajukan_oleh`, `disetujui_oleh`, `disetujui_at`, `alasan_pengajuan` (migration `2026_09_12_153112_add_pengajuan_fields_to_home_visits.php`). `siswa_id` stores `idperson` (FK dropped, same as `penanganan`).
- Admin list/riwayat lives at `admin/home-visit` (route `admin.home-visit.index`, view `admin/home-visit/index.blade.php` + `partials/list.blade.php`); `admin.home-visit.select` is still the student picker for creating one directly.
- Eligibility unit tests: `tests/Unit/HomeVisitEligibilityServiceTest.php` (Mockery-based, runnable — no DB). `tests/Pest.php` binds `Unit` to the full `TestCase` but without `RefreshDatabase`.

### External integration

- `App\Services\SiswaService::updateTelepon()` POSTs to `https://api.daruttaqwa.or.id/sisda/v1/update_telepon/{idperson}` using a cookie `SWN` from the `API_SWN` env var (not yet present in `.env.example`).
- There is a secondary `mysql_second` DB connection defined in `config/database.php` (env `DB_*_SECOND`); it is only referenced in `CustomController`, and several commented-out uses remain in `web.php`.

### Commands & workflow

- `composer test` runs `artisan config:clear` + `artisan test` (Pest). Run a single test with `vendor/bin/pest --filter=...` (or `lerd exec vendor_run`).
- Code style: `vendor/bin/pint`. Format before committing.
- Frontend: `npm run build` (Vite) / `npm run dev` (HMR via the lerd `vite` host worker). Tailwind v4 via `@tailwindcss/vite`.
- There is a scheduled job `pembayaran:refresh-status-lunas` (every 6h, `routes/console.php`) that recomputes `siswa_status_pembayaran` from `daruttaqwa_trans` — this is why "lunas" status can lag behind real payments; run `php artisan pembayaran:refresh-status-lunas` (or with `{idperson}`) to force a refresh.
- `app/Exports/SiswaBelumLunasExport copy.php` is a stray duplicate of `SiswaBelumLunasExport.php` — ignore/delete it, don't edit it.

### Existing instruction files

- The `<!-- lerd:begin --> … <!-- lerd:end -->` block above is machine-generated by lerd; don't hand-edit it. Related lerd tooling docs live in `GEMINI.md`, `.github/copilot-instructions.md`, `.cursor/rules/lerd.mdc`, and `.claude/skills/lerd/SKILL.md`.
